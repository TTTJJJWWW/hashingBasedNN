...
flags.DEFINE_string("save_dir", "./save/lh/", 
...
save_dir_adv_training = os.path.join(FLAGS.save_dir, 'adv_train/')
    ...
    with sess.as_default():    
        for epoch in range(FLAGS.epoches):
            ...
            for mini_batch, trainX_batch, trainy_batch in train_input.next_batch():
                ...
                #select malware samples
                trainX_adv_batch = trainX_batch.copy()
                mal_samples_idx = (trainy_batch[:,1] == 1.)
                ...
                test_adv_acc = np.mean(np.array(test_adv_accs))
...
def adversarial_train(sess, model, train_input, test_input, test_adv_input, config):
    #training
    best_iter = 0
    best_acc = 0.
    MSG = "training start at {0}"
    ...
    start_time = datetime.datetime.now()
    saver = tf.train.Saver()
    if not os.path.exists(save_dir_adv_training):
        os.mkdir(save_dir_adv_training)    
    log = open(os.path.join(save_dir_adv_training, 'adv_acc_log.txt'), 'w')
    train_bias = np.array([[10., 1.]], dtype = np.float32)
    test_bias = np.array([[1., 1]], dtype = np.float32)
            ...
            for mini_batch, trainX_batch, trainy_batch in train_input.next_batch():
                ...
                if iterations % 100 == 0:
                    ...
                    if best_acc < _acc_ceri:
                        ...
                        print(MSG.format(best_acc, best_iter, test_acc))
                        #save model parameters
                        save_model_path = os.path.join(save_dir_adv_training + "model.ckpt")
                        saver.save(sess, save_model_path)
...
def test(name = 'train'):
    if name == 'train' or name == 'adv_train':
        #read dataset
        ...
        if name == 'train':
            ...
            adversarial_train(sess, model_mlp, train_input, test_input, test_adv_input, config)
    if name == 'evaluate':
...
def main(_):
    ...
    mal_data = utils.readdata_np(MAL_DATA_PATH)
    ben_data = utils.readdata_np(BEN_DATA_PATH)
    dataX = np.vstack([mal_data, ben_data])
    y = np.zeros((dataX.shape[0], 2))
    y[:mal_data.shape[0], 0] = 1.
    y[mal_data.shape[0]:, 1] = 1.
    evaluate(sess, model_mlp, dataX, y)
    '''
