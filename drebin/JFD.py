...
flags.DEFINE_string("save_dir", "./save/jfd/", 
...
def random_noises_dae(batch_size, onehot_dim, input_dim):
    ...
    return np.random.binomial(1, prob, (batch_size, FLAGS.L, onehot_dim))

def random_noises_vir(batch_size,  onehot_dim):
    return np.zeros((batch_size, FLAGS.L, onehot_dim))
            ...
            for epoch in range(n_epoches_dae):
                ...
                for mini_batch, trainX_batch, trainy_batch in train_input.next_batch():
                    train_dict = {model.x : trainX_batch,
                                  model.noises: random_noises_dae(trainX_batch.shape[0], trainX_batch.shape[2], config.input_dim[2]),
        ...
        def _evl(X, y):
            ...
            for mini_i in range(batches):
                ...
                _feed_dict ={
                        ...
                        model.noises: random_noises_vir(end_i - start_i, X.shape[2]),
                        model.is_training: False,
                        model.bias: test_bias
                        }
...
def test(name = 'train'):
    if name == 'train':
        ...
        config.input_dim.append(trainX.shape[1])
        
        tf.reset_default_graph()
        
        model_mlp = FFN_lhrf_dae_model(config)
            ...
            def _evl(X, y):
                ...
                for mini_i in range(batches):
                    ...
                    _feed_dict ={
                            ...
                            model_mlp.noises: random_noises_vir(end_i - start_i, X.shape[2]),
